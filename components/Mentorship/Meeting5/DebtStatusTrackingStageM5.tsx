...
interface DebtPriorityStatus {
    ...
    observation: string;
    date: string;
    previousObservation?: string; // Observação da Reunião 4
}

interface DebtStatusTrackingStageM5Props {
    meetingData: any;
    m4Data: any;
    m3Data: any;
...
export const DebtStatusTrackingStageM5: React.FC<DebtStatusTrackingStageM5Props> = ({
    meetingData,
    m4Data,
    m3Data,
    onUpdateMeetingData,
    ...
    useEffect(() => {
        ...
        // and Meeting 4 (Source of observations)
        ...
        if (!hasExistingStatus && m3Data) {
            ...
            if (debtsToTrack.length > 0) {
                const initialItems: DebtPriorityStatus[] = debtsToTrack.map((d: any) => {
                    // Try to find previous observation and status from M4
                    ...
                    return {
                        ...
                        name: d.name,
                        creditor: d.creditor,
                        status: m4Status?.status || 'Não Iniciado',
                        observation: '', // New observation for M5
                        previousObservation: m4Status?.observation || '', // Reference from M4
    ...
    const handleRefresh = () => {
        ...
        if (currentItem) {
            // Update reference values and inherit from M4 if M5 is still empty
            const updatedItems = statusItems.map(i =>
                i.id === sourceDebt.id
                    ? {
                        ...
                        previousObservation: m4Status?.observation || i.previousObservation
                    }
                    : i
            );
            ...
            if (confirm("Deseja sincronizar com a dívida prioritária da Reunião 3? O histórico atual desta etapa na M5 será substituído.")) {
                const newItem: DebtPriorityStatus = {
                    ...
                    status: m4Status?.status || 'Não Iniciado',
                    observation: '',
                    previousObservation: m4Status?.observation || '',
                    date: new Date().toISOString().split('T')[0]
                };
        ...
        <div className="space-y-6 animate-fade-in relative pb-10 print:pb-0">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-xs text-slate-500 font-bold uppercase tracking-wider mt-1 print:text-gray-500">
                        Evolução da dívida selecionada na Reunião 3 com histórico da Reunião 4
                    </p>
                </div>
                {!readOnly && (
                    <button
                        ...
                        title="Sincronizar com Reunião 3 e 4"
                    >
                        <RefreshCw size={14} />
                        Sincronizar Dados
                    </button>
            ...
            {statusItems.length === 0 ? (
                <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-12 text-center">
                    ...
                    <p className="text-xs text-slate-600 mt-2 uppercase font-bold">Inicie o plano na Reunião 3 e acompanhe na Reunião 4 para visualizar aqui.</p>
                </div>
            ) : (
                        ...
                        <div key={item.id} className="bg-slate-900/40 border border-slate-800 rounded-[1.5rem] p-6 hover:border-slate-700 transition-all group print:bg-white print:border-gray-200 print:break-inside-avoid">
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                ...
                                <div className="flex flex-wrap items-center gap-6">
                                    ...
                                    <div className="space-y-1.5 min-w-[160px]">
                                        <label className="text-[10px] text-slate-500 uppercase font-black px-1">Status na R5</label>
                            ...
                            <div className="mt-6 pt-6 border-t border-slate-800/50">
                                <div className="flex items-start gap-3">
                                    ...
                                    <textarea
                                        placeholder="Novas observações para esta Reunião 5..."
            ...
            {!readOnly && statusItems.length > 0 && (
                <div className="flex justify-end pt-8 border-t border-slate-800 print:hidden">
                    <button onClick={handleSave} className="bg-purple-500 hover:bg-purple-600 text-white px-8 py-3 rounded-xl font-bold flex items-center gap-2 transition-all shadow-lg shadow-purple-500/20 uppercase text-xs tracking-widest">
                        {showSuccess ? <><CheckCircle size={18} /> Salvo!</> : <><Save size={18} /> Salvar Status R5</>}
                    </button>
                </div>
            )}
        </div>
