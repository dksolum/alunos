...
interface DreamsGoalsStageM5Props {
    meetingData: any;
...
export const DreamsGoalsStageM5: React.FC<DreamsGoalsStageM5Props> = ({
    meetingData,
    previousMeetingData,
    onUpdateMeetingData,
    readOnly = false
    ...
    // Inherit from Meeting 4 if current is empty
    useEffect(() => {
                    ...
                    <div className="flex gap-2 items-center">
                        ...
                        {previousMeetingData?.dreamsGoals && previousMeetingData.dreamsGoals.length > 0 && (
                            <button
                                ...
                                title="Sincronizar alterações feitas na Reunião 4"
                            >
                                <RefreshCw size={16} className={isSyncing ? "animate-spin" : ""} />
                                {isSyncing ? 'Sincronizando...' : 'Sincronizar M4'}
                            </button>
                        )}
                        {goals.length >= 2 && (
                            <button
