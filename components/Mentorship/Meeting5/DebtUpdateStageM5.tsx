...
interface DebtUpdateItem {
    ...
    origin?: 'mapping' | 'meeting2' | 'meeting3' | 'meeting4' | 'meeting5';
    updatedAt?: string;
    createdAt?: string;
    endDate?: string;
}

interface DebtUpdateStageM5Props {
    userId: string;
...
export const DebtUpdateStageM5: React.FC<DebtUpdateStageM5Props> = ({
    userId,
    checklistData,
    meetingData,
    previousMeetingData,
    ...
    const fetchAndMergeDebts = async (isManualRefresh = false) => {
        ...
        try {
            ...
            if (isManualRefresh) {
                ...
                if (m4) m4Data = m4.data;
            }

            // Priority 1: Meeting 4 Data
            let meeting4Debts: any[] = m4Data?.debtUpdates || [];

            // Fallback: This should ideally not happen if M4 was done
            if (meeting4Debts.length === 0) {
                const mapping = await authService.getDebtMapping(userId);
                meeting4Debts = mapping.map(d => ({
                    id: d.id,
                    name: d.name,
                    creditor: d.creditor,
            ...
            const syncedDebts: DebtUpdateItem[] = meeting4Debts.map(d => {
                ...
                return {
                    ...
                    newInstallment: useLocalM5 ? (existingM5?.newInstallment ?? (d.newInstallment || d.originalInstallment || 0)) : (d.newInstallment || d.originalInstallment || 0),
                    originalQuantity: d.newQuantity || d.originalQuantity || 0,
                    newQuantity: useLocalM5 ? (existingM5?.newQuantity ?? (d.newQuantity || d.originalQuantity || 0)) : (d.newQuantity || d.originalQuantity || 0),
                    originalInterest: d.newInterest || d.originalInterest || '0%',
                    newInterest: useLocalM5 ? (existingM5?.newInterest ?? (d.newInterest || d.originalInterest || '0%')) : (d.newInterest || d.originalInterest || '0%'),
                    isNegotiated: d.isNegotiated, // Inherit from M4
                    isManual: false,
                    isPaid: useLocalM5 ? (existingM5?.isPaid !== undefined ? existingM5.isPaid : (d.isPaid !== undefined ? d.isPaid : true)) : (d.isPaid !== undefined ? d.isPaid : true),
                    amortizationConfirmed: useLocalM5 ? (existingM5?.amortizationConfirmed || false) : false,
                    origin: d.origin || 'mapping',
                    createdAt: d.createdAt || now,
                    updatedAt: now,
                    endDate: calculateEndDate(useLocalM5 ? (existingM5?.newQuantity || d.newQuantity || d.originalQuantity || 0) : (d.newQuantity || d.originalQuantity || 0))
                };
            });

            const syncedIds = new Set(syncedDebts.map(d => d.id));
            const meeting5ManualDebts = debts.filter(d => d.isManual && !syncedIds.has(d.id));
            ...
            console.error('Error fetching/merging debts in M5:', error);
        } finally {
            setLoading(false);
            setRefreshing(false);
        }
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
                ...
                <p className="text-slate-400 font-medium">Sincronizando com Reunião 4...</p>
            </div>
        );
    }
